...
class TTAMethod(nn.Module):
    ...
    def forward(self, x, is_source: bool = False):
        if self.episodic:
            self.reset()
        ...
        if x[0].shape[0] == 1:  # single sample test-time adaptation
            ...
            if self.pointer == (self.window_length - 1):
                ...
                for _ in range(self.steps):
                    outputs = self.forward_and_adapt(self.input_buffer, is_source=is_source)
            ...
            for _ in range(self.steps):
                outputs = self.forward_and_adapt(x, is_source=is_source)
