...
def create_cifarc_dataset(
    ...
    x_test = torch.tensor([])
    y_test = torch.tensor([])
    corruptions_seq = corruptions_seq if "continual_cdc" in setting else [corruption]
...
def create_imagenetc_dataset(
    ...
    if "continual_cdc" in setting or "correlated" in setting or n_examples != -1:
        ...
        if n_examples != 5000 and (n_examples != -1 or "correlated" in setting):
        ...
        logger.info(f"Use file path {file_path}")
        ...
        for cor in corruptions_seq:
            ...
            item_list += [(os.path.join(corruption_dir_path, fn.split('\n')[0]), class_to_idx[fn.split(os.sep)[0]], cor) for fn in fnames]
